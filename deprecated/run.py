...
def get_dev_name(dev_type, name, **kwargs):
    ...
    if dev_type == 'switch':
        ...
        prefix = '' if kwargs.get('is_peer', False) else 'v'
...
def setup_bridge_interface(ns_name, dev_type, switch, cidr):
    # peer will be added into netns
    peer_name = get_dev_name(dev_type, ns_name, switch=switch, is_peer=True)
    if cidr.endswith('/32'):
        cmds = [add_dev_cidr(peer_name, cidr, ns_name)]
    else:
...
def setup_bridge_node(name, info):
    ret = [
        ...
        for (switch, cidrs) in info['interfaces'].iteritems()
...
def pre_check(topo_d, name, info):
    for switch, cidrs in info.get('interfaces', {}).iteritems():
        ...
        if topo_d[name]['type'] == 'ovn_node':
            cidrs = cidrs.split()[1:]
        else:
            cidrs = cidrs.split(',') if ',' in cidrs else cidrs.split()
        for cidr in cidrs:
            ip = netaddr.IPNetwork(cidr.split('/')[0])
            if not (net.first < ip._value < net.last):
                raise ValueError(
                    "%(name)s interface cidr %(cidr)s doesn't match its "
                    "%(switch)s" % {
                        'name': name, 'switch': switch, 'cidr': cidr})
    ...
    try:
        for name, info in topo_d.iteritems():
            if name in ('services', 'description'):
                continue
            if name == 'modprobe':
                remove = "" if method == "setup" else "-r"
                cmds.insert(0, '\n'.join([
                    "modprobe %s %s" % (remove, mod) for mod in topo_d[name]]))
                continue
            _type = info['type']
            ...
            if method == 'setup':
                ...
                cmd = (
                    'pids=`ip netns pids %s`; if [[ $pids != "" ]] ; then kill'
                    ' -9 $pids; fi\n' % name  + cmd)
            if top:
                cmds.insert(0, cmd)
            else:
...
if __name__ == '__main__':
    ...
    if os.getuid() != 0:
        print 'Need run as root, for operating with linuxbridge and netns'
        os.sys.exit(1)
        ...
        if os.path.exists(LOCK) and method == 'setup' and not debug:
            print (
                ...
                "you confirm there is no topo yet" % {
                    'topo': open(LOCK).read(), 'lock': LOCK})
            os.sys.exit(1)
        elif not os.path.exists(LOCK) and method == 'destroy' and not debug:
